...
static int oled_bsp_init(void)
{
    ...
    /* SCLK */
    uapi_pin_set_mode(15, HAL_PIO_SPI1_CLK);

    /* MOSI */
    uapi_pin_set_mode(14, HAL_PIO_SPI1_TXD);
    ...
    uapi_gpio_set_val(11, GPIO_LEVEL_HIGH);

    /* 初始化SPI */
    spi_attr_t config = { 0 };

    config.freq_mhz = 4;                                       /* 分频值 32MHz / 32 = 1MHz */
    config.is_slave = false;                                    /* 主机模式 */
    config.frame_size = HAL_SPI_FRAME_SIZE_8;                   /* 帧大小，8位 */
    config.slave_num = 1;                                       /* 片选 0 */
