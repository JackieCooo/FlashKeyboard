...
static uint8_t ble_set_adv_name(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ...
    adv_local_name_data.length = (uint8_t)(BLE_ADV_PARAM_DATATYPE_LENGTH + sizeof(CONFIG_KEYBOARD_NAME));
    ...
    n_ret = memcpy_s((set_adv_data_position + len), (size_t)(max_len - len), CONFIG_KEYBOARD_NAME, sizeof(CONFIG_KEYBOARD_NAME));
    if (n_ret != EOK) {
        return 0;
    }
    len = (uint8_t)(len + sizeof(CONFIG_KEYBOARD_NAME));
    return len;
}
...
static uint16_t ble_set_scan_response_data(uint8_t *scan_rsp_data, uint8_t scan_rsp_data_max_len)
{
    ...
    /* set local name */
    scan_rsp_data[idx++] = sizeof(CONFIG_KEYBOARD_NAME) + BLE_GENERAL_BYTE_1;
    scan_rsp_data[idx++] = BLE_ADV_LOCAL_NAME_DATA_TYPE;
    if ((idx + sizeof(CONFIG_KEYBOARD_NAME)) > scan_rsp_data_max_len) {
        return 0;
    }
    n_ret = memcpy_s(&scan_rsp_data[idx], scan_rsp_data_max_len - idx, CONFIG_KEYBOARD_NAME, sizeof(CONFIG_KEYBOARD_NAME));
    if (n_ret != EOK) {
        return 0;
    }
    idx += sizeof(CONFIG_KEYBOARD_NAME);
    return idx;
}
